...
func TestIsStatusBroadcastChatJID(t *testing.T) {
	tests := []struct {
		...
		{chatJID: "status@broadcast", want: true},
		{chatJID: "STATUS@BROADCAST", want: true},
		{chatJID: "status@newsletter", want: true},
		{chatJID: "120363123@g.us", want: false},
		{chatJID: "628123456789@s.whatsapp.net", want: false},
	}

	for _, tt := range tests {
		got := isStatusBroadcastChatJID(tt.chatJID)
		if got != tt.want {
			t.Fatalf("isStatusBroadcastChatJID(%q)=%v want=%v", tt.chatJID, got, tt.want)
		}
	}
}
